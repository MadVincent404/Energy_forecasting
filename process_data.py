...
def perform_feature_engineering(file_path, train_name, test_name, lags, windows):
    ...
    # Renommage en premier pour que le feature engineering utilise les bons noms
    df = df.rename(columns={
        'temperature_moyenne': 'Température moyenne (°C)',
        'temperature_reference': 'Température référence (°C)',
    })
    ...
    features = ["pic_journalier_consommation", "Température référence (°C)", "Température moyenne (°C)"]

    for feature in features:
        cleaned_name = feature.replace(' (°C)', '').replace(' (MW)', '').replace(' ', '_')

        for lag in lags:
            df[f"{cleaned_name}_lag_{lag}"] = df[feature].shift(lag)

        for window in windows:
            df[f"{cleaned_name}_roll_{window}"] = df[feature].rolling(window=window).mean().shift(1)

    max_lag = max(lags)
    df = df.dropna(subset=[f"pic_journalier_consommation_lag_{max_lag}"])

    nb_datas = len(df)
    nb_tail = 90
    df.head(nb_datas - nb_tail).to_csv(train_name)
    df.tail(nb_tail).to_csv(test_name)
    logging.info(f"Train : {nb_datas - nb_tail} lignes → {train_name}")
    logging.info(f"Test  : {nb_tail} lignes → {test_name}")
...
if __name__ == "__main__":
    ...
    if not data_path.exists():
        logging.error(f"Fichier manquant : {data_path}")
        exit(1)
